...
def contratos_por_vencer(institucion,dias=30):
 hoy=timezone.localdate();return ContratoLaboral.objects.filter(institucion=institucion,estado=ContratoLaboral.Estado.VIGENTE,fecha_fin__range=(hoy,hoy+timedelta(days=dias))).select_related("empleado","puesto")
...
def registrar_egreso(empleado,fecha,motivo,usuario,desactivar_usuario=True):
 empleado=Empleado.objects.select_for_update().get(pk=empleado.pk);empleado.estado=Empleado.Estado.RETIRADO;empleado.fecha_egreso=fecha;empleado.save();mov=MovimientoLaboral.objects.create(institucion=empleado.institucion,empleado=empleado,fecha=fecha,tipo=MovimientoLaboral.Tipo.EGRESO,puesto_anterior=empleado.puesto,area_anterior=empleado.area,descripcion=motivo,registrado_por=usuario)
...
def resolver_permiso(request,permiso,estado):
 ...
 if estado not in (PermisoLaboral.Estado.APROBADO,PermisoLaboral.Estado.RECHAZADO):raise ValidationError("Resolución inválida.")
